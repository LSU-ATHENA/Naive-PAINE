...
from .model_registry import SDXL_CONFIG, load_sdxl_pipeline


class SDXLGenerator(BaseDataGenerator):
    def __init__(self, save_dir, prompts, **kwargs):
        super().__init__(
            save_dir=save_dir, prompts=prompts,
            latent_shape=SDXL_CONFIG['latent_shape'], **kwargs,
        )
        self.guidance_scale = SDXL_CONFIG['guidance_scale']
        self.num_inference_steps = SDXL_CONFIG['num_inference_steps']

    def setup_pipeline(self):
        return load_sdxl_pipeline(device=self.device)
